...
int eventdisplay(const char * fname = "../wcsim.root")
{
    ...
    m2.Draw();
    c1->SaveAs(Form("plots/%sdisplay.pdf",prefix.c_str()));
    ...
    c1->SaveAs(Form("plots/%stimetof.pdf",prefix.c_str()));

    MakeWhiteAxis(hist_timetof_true);
    hist_timetof_true->Draw("hist");
    //c1->SetLogy();
    c1->SaveAs(Form("plots/%stimetof_true.pdf",prefix.c_str()));

    MakeWhiteAxis(hist_NDigiHits);
    hist_NDigiHits->Draw("hist");
    c1->SaveAs(Form("plots/%sNDigiHits.pdf",prefix.c_str()));
